...
class WaterMazeGenerator(MazeGenerator):
    def __init__(self, radius_maze, radius_platform, obstacle_ratio=0.0):
        ...
        self.platform[circle(6, 7, self.radius_platform)] = 3  # TODO srini: this can cause problem for smaller mazes
    ...
    def _generate_obstacles(self):
        ...
        obstacles = np.zeros(self.maze.shape)
        # Randomly sample locations according to obstacle_ratio
        _r = (self.radius_maze * 2) ** 2
        random_idx_idx = np.random.choice(_r, size=int(self.obstacle_ratio * _r), replace=False)
        random_obs_idx = all_idx[random_idx_idx]

        # Fill obstacles
        for idx in random_obs_idx:
            if self.maze[idx[0], idx[1]] != 3 and self.maze[idx[0], idx[1]] != 1:  # Don't add obstacles on goal states
                obstacles[idx[0], idx[1]] = 1
        return obstacles

    def get_maze(self):
        # generate obstacles
        if self.obstacles is not None:
            self.maze -= self.obstacles
